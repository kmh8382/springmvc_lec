...
@Controller   // 컨드롤러가 사용하는 @Component 입니다.
...
public class ContactController {
  ...
  public String list(Model model) {
    ...
    // JSP 경로를 작성합니다.
    return "contact/list";    // /WET-INF/views/contact/list.jsp
  }
  
  @RequestMapping(value="/detail.do")
  public String detail(HttpServletRequest request, Model model) {
    Optional<String> opt = Optional.ofNullable(request.getParameter("contact_id"));
    int contact_id = Integer.parseInt(opt.orElse("0"));   // 요청 파라미터 contact_id가 null 이면 "0"을 사용합니다.
  ...
  public String modify(HttpServletRequest request, RedirectAttributes redirectAttributes) {
    ...
    // 수정 성공/실패 메시지를 RedirectAttributes에 저장합니다. Model에 저장하면 리다이렉트할 때 전달되지 안습니다.
    redirectAttributes.addFlashAttribute("msg", modifyMsg);
    // 연락처 목록 보기로 리다이렉트 합니다. 삽입/수정/삭제 이후에는 반드시 리다이렉 합니다.
  ...
  public String remove(HttpServletRequest request, RedirectAttributes redirectAttributes) {
    // 요청을 그대로 삭제 서비스로 전달하고, 삭제 성공/실패 메시지를 받아옵니다.
  ...
  public String register(HttpServletRequest request, RedirectAttributes redirectAttributes) {
    // 요청을 등록 서비스로 전달한 뒤 등록 성공/실패 메시지를 받아와서 RedirectAttributes 에 저장합니다.
