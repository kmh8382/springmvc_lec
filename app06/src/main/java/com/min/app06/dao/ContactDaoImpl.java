 ...
 * 3. 기본적으로  ...  으로 생성합니다. (스프링은 Spring Container에 bean을 만들 때 기본적으로 Single  ...  )
 */

@Repository   // DAO를 구현한 클래스라는 의미를 가진 @component입니다. (Spring Container에 bean으로 ) 
              // Persistencee Layer(영속 계층)에 존재합니다.
...
public class ContactDaoImpl implements IContactDao {
  ...
  public ContactDto getContactById(int contact_id) {
    ...
    try {
      ...
      ps = conn.prepareStatement(sql);
      // 쿼리문에 물음표(위치 홀더)가 존재하면 해당 위치에 값을 전달해야합니다.
      // 각 물음표(위치 홀더)의 구분은 숫자로 합니다. 첫 번째 물음표의 위치는 1입니다. 
      ps.setInt(1, contact_id);  // 첫 번째 물음표(위치 홀더)에 contact_id를 전달합니다.
      // 쿼리문을 실행하고 결과를 ResultSet으로 받습니다.
      rs = ps.executeQuery();
      // 결과 행은 - 또는 1개이므로 if문으로 결과 행의 존재 여부를 파악합니다.
      if(rs.next()) {
        // 결과 행(Row)의 각 열(Column) 값을 가져가져와서 ContactDto 객체로 만듭니다.
        // @Builder(빌더 패턴)을 이용해 보겠습니다.
