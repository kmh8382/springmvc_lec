  ...
  <table border="1">
    ...
    <tbody>
      <c:forEach var="blog" items="${blogList}" varStatus="vs">
        <tr class="blogs" data-blog-id="${blog.blog_id}" data-user-email="${blog.user_email}">
  ...
  <script>
    ...
    for(const blog of blogs) {
      blog.addEventListener('click', (event) => {
        // data-user-email="${blog.user_email}를 이용하여 필요한 데이터를 가져온다.
        // event.currentTarget.dataset.userEmail 방식으로 사용한다.
        if('${sessionScope.loginUser.userEmail}' === event.currentTarget.dataset.userEmail) {
          location.href = '${contextPath}/blog/detail.do?blog_id=' + event.currentTarget.dataset.blogId;
        } else {
