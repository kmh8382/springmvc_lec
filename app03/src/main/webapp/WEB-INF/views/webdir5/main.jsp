...
<script>
  window.onload = () => {   // window 객체는 항상 생략할 수 있습니다. onload = () => {} 로 작성해도 됩니다.
    ...
    btn1.addEventListener('click', (event) => {
      ...
      // JSP 에서는 템플릿 문자열을 사용할 때 \${} 를 사용합니다.
      location.href = `\${contextPath}/webdir5/req1?param=\${event.currentTarget.textContent}`;  // 클릭한 요소(event.currenTarget)의 내부 텍스트(textContent)
    ...
    btn2.addEventListener('click', (event) => {
      ...
      const contextPath = '<%=request.getContextPath()%>';
      location.href = `\${contextPath}/webdir5/req1?param=\${event.currentTarget.dataset.id}`;
    ...
    for(const btn of btnDo) {
      btn.addEventListener('click', (event) => {
        ...
        // Element : 한마디로 태그(HTMLElement)를 의미하는 Node 의 하위 개념 DOM 객체입니다.
...
<body>
  ...
  <%-- <a> 태그를 클릭하면 JavaScript 의 fnMovw() 함수가 호출됩니다. --%>
  <%-- this : 클릭한 요소를 의미합니다. 어떤 <a> 태그를 클릭했는지 함수에 인자로 전달합니다. --%>
  <a onclick="fnMove(this)" data-url="https://www.naver.com">네이버</a>
