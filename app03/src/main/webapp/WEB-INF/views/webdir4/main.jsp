...
<body>
  ...
  <form action="<%=request.getContextPath()%>/webdir4/req1" method="get">  
    ...
    <%-- 서브밋(action 에 작성된 주소로 모든 입력 요소글을 보냅니다. --%>
    <button type="submit">서브밋</button>    
  </form>
  <hr/>
  
  
  <form action="<%=request.getContextPath()%>/webdir4/req2">
    <%-- 모든 선택 상자는 선택했을 경우 디폴트 "value=on" 을 가집니다. --%>
  
    <%-- 선택 상자 : 다중선택 가능한 checkbox (같은 name 으로 보내기) - 디폴트 "value=on" 을 사용할 수 없습니다. --%>
    ...
    <%-- 선택 상자 : 다중선택 가능한 checkbox (다른 name 으로 보내기) - 디폴트 "value=on" 을 사용할 수 있습니다. --%>
    ...
    <%-- 선택 상자 : 단일선택 가능한 radio - 디폴트 "value=on" 을 사용할 수 없습니다. --%>
    <input type="radio" name="choice" id="yes" value="y">
    <label for="kbs">yes</label>
  ...
  <form action="<%=request.getContextPath()%>/webdir4/req3">
    <%-- 모든 목록 요소는 <option> 태그의 내부 텍스트 가 value 로 전달됩니다. --%>
    <%-- <option> 태그에 value 속성을 추가하면 value 속성 값이 전달됩니다. --%>
  
    <%-- 목록 요소 : <select> 태그 와 <option> 태그 --%>
    <select name="city">
      <option>seoul</option>
      <option>jeju</option>
    </select>
